...
with st.sidebar:
    ...
    provider = st.selectbox("Proveedor LLM", ["Auto (Groq→HF)", "Groq", "Hugging Face"])
...
def preprocess(img, contrast_boost=25, sharpen=True):
    ...
    if contrast_boost and contrast_boost > 0:
        img = ImageEnhance.Color(img).enhance(0.0)  # BN
...
def analyze_with_groq(prompt: str, model: str, temperature: float, max_tokens: int):
    ...
    try:
        # Lazy import to avoid hard dependency if user uses HF
...
def analyze_with_hf(prompt: str, model: str, temperature: float, max_tokens: int):
    ...
    data = r.json()
    # HF can return list of dicts with 'generated_text'
    if isinstance(data, list) and data and "generated_text" in data[0]:
        return data[0]["generated_text"]
    # Some TGI servers return dict
...
if uploaded:
    ...
    # Concatenate lines sorted by y position if available
    try:
        import numpy as np
    ...
    if run_llm:
        ...
        if provider_choice in ("Auto (Groq→HF)", "Groq"):
            try:
        ...
        if (response_text is None) and (provider_choice in ("Auto (Groq→HF)", "Hugging Face")):
            try:
